...
import { buildProfileSearchParams, parseProfileSearchParams } from '@/lib/url-state'
import {
  buildRegionStats,
  countIndependent,
...
export default function ProfileContent() {
  ...
  const [initialUrlState] = useState(() => parseProfileSearchParams(searchParams))
  ...
  const [compareASelection, setCompareASelection] = useState(initialUrlState.compareA)
  const [compareBSelection, setCompareBSelection] = useState(initialUrlState.compareB)
  ...
  // Keeps the URL in sync so the current filter/sort/compare state is
  // shareable and bookmarkable. Uses replace (not push) so typing in the
  // search box doesn't spam browser history.
  useEffect(() => {
  ...
  }, [query, selectedRegion, sortBy, compareA, compareB, pathname, router])
